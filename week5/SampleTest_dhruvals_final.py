...
def test_case():
 ...
 [4., 4., 4.,]]), decimal=2)
    ...
    try:
        a=Tensor(np.array([[1.,2.,3.],[4.,5.,6.],[6.,64.,65.]]))
        b=Tensor(np.array([[4.,5.,6.],[2.,3.,4.]]))
        a.zero_grad()
        b.zero_grad()
        c=(b@a)+b
        c.backward()
        np.testing.assert_array_almost_equal(a.grad, np.array([[ 6., 6. ,6.],[ 8. ,8. ,8.],[10.,10. ,10.]]), decimal =2)
        np.testing.assert_array_almost_equal(b.grad, np.array([[  7.,16.,136.],[  7. ,16., 136.]]), decimal =2)
        print("Test Case 15 \033[92mPASSED\033[0m")
    except Exception as e:
        print("Test Case 15 \033[91mFAILED\033[0m", e)
        traceback.print_exc()
        
    
    try:
        a = Tensor(np.array([[6., 5., 1.],[1., 0., 5.],[6., 6., 6]]))
        b = Tensor(np.array([[1., 1., 1.], [1., 1., 1.], [1., 1., 1.]]))
        a.zero_grad()
        b.zero_grad()

        mul = (a + a) @ b @ a @ a @ (a + a @ b) + b
        
        mul.backward()
        

        np.testing.assert_array_almost_equal(a.grad, np.array([[168048., 140400., 218160.,],
 [154224., 130032., 200880.,],
 [158544., 132624., 206928.,]]), decimal=2)
        
        np.testing.assert_array_almost_equal(b.grad, np.array([[290281., 264697., 413209.],
 [245689., 224041., 349705.],
 [260209., 236593., 373681.]]))
        
        print("Test Case 16 \033[92mPASSED\033[0m")
    except Exception as e:
        print("Test Case 16 \033[91mFAILED\033[0m", e)
        traceback.print_exc()
        
    try:
        a1 = Tensor(np.array([[6., 5., 1.],[1., 0., 5.],[6., 6., 6]]))
        b = Tensor(np.array([[1., 1., 1.], [1., 1., 1.], [1., 1., 1.]]))
        a1.zero_grad()
        b.zero_grad()

        mul = ((a1 + a1) @ b @ a1 @ a1 @ (a1 + a1 @ b) + b) @ b @ b + a1
        
        mul.backward()
        

        np.testing.assert_array_almost_equal(a1.grad, np.array([[1512433., 1263601., 1963441.,],
 [1388017., 1170289., 1807921.,],
 [1426897., 1193617., 1862353.,]],), decimal=2)
        
        np.testing.assert_array_almost_equal(b.grad, np.array([[5690331., 5460075., 6796683.],
 [5224635., 5029803., 6160779.],
 [5364171., 5151627., 6385419.]]))
        
        print("Test Case 17 \033[92mPASSED\033[0m")
    except Exception as e:
        print("Test Case 17 \033[91mFAILED\033[0m", e)
        traceback.print_exc()
